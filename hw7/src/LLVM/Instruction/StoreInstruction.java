...
public class StoreInstruction extends Instr {
    ...
    public void toAssembly() {
        ...
        else if (MipsBuilder.getInstance().getRegByValue(to.getName()) != null) {
            toReg = MipsBuilder.getInstance().getRegByValue(to.getName());
        }
        else {
            //从内存中获取
            //TODO 一定保证内存中有吗？
        ...
        if (from instanceof Init) {
            ...
            for (i = 0 ; i < length; i++) {
                ...
                } else if (MipsBuilder.getInstance().getRegByValue(from1.getName()) != null) {
                    fromReg = MipsBuilder.getInstance().getRegByValue(from1.getName());
                } else {
        ...
        } else if (MipsBuilder.getInstance().getRegByValue(from.getName()) != null) {
            fromReg = MipsBuilder.getInstance().getRegByValue(from.getName());
            new MemAsm(MemAsm.Op.SW, fromReg, toReg, 0);
        } else {
