        ...
        //局部变量
            ...
            if (initFlag) {
                ...
                if (((TokenNode)getChildren().get(1)).getToken().getType() == TokenType.ASSIGN) {
                    ...
                    LocalVar localVar = new LocalVar(llvmType,name,isArray,InitVal,targetType,new ArrayList<>(),allocaInstr);
                    //store
                ...
                //Ident { '[' ConstExp ']' } '=' InitVal
                    ...
                    LocalVar localVar = new LocalVar(LLVMType.LOCAL_VAL,name,isArray,constInitVal,targetType,initDims,allocaInstruction);
                    //store
                    //TODO
            ...
            //无初始值
                ...
                if (getChildren().size() == 1) {
                    ...
                    IRBuilder.getInstance().addInstr(allocaInstruction);
                    LocalVar localVar = new LocalVar(llvmType,name,isArray,valueArrayList,targetType,new ArrayList<>(),allocaInstruction);
                    //加入符号表
                    VarSymbol varSymbol = new VarSymbol(varName,SymbolType.var,0);
                    varSymbol.setIr(localVar);
                    varSymbol.setDims(new ArrayList<>());
                ...
                //VarDef → Ident { '[' ConstExp ']' }
                    ...
                    IRBuilder.getInstance().addInstr(allocaInstruction);
                    ArrayList<Value> valueArrayList = new ArrayList<>();
                    LocalVar localVar = new LocalVar(llvmType,name,isArray,valueArrayList,targetType,initDims,allocaInstruction);
                    //加入符号表
                    VarSymbol varSymbol = new VarSymbol(varName,SymbolType.var,1);
                    varSymbol.setIr(localVar);
                    varSymbol.setDims(initDims);
