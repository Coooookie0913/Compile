...
public class GetElementPtrInstruction extends Instr {
    ...
    public void toAssembly() {
        ...
        else if (MipsBuilder.getInstance().getRegByValue(pointer) != null) {
            pointerReg = MipsBuilder.getInstance().getRegByValue(pointer);
        }
        else {
        ...
        if (offset instanceof Constant || flag == 0) {
            ...
            //offset在寄存器中
            if (MipsBuilder.getInstance().getRegByValue(offset) != null) {
                offsetReg = MipsBuilder.getInstance().getRegByValue(offset);
            }
            //offset在栈上
            else {
